...
float midpoint = (REFLECT_WHITE - REFLECT_BLACK) / 2 + REFLECT_BLACK; //62.5
...
int DetectionColor(int color_sensor){
    ...
    if(midpoint-5 < reflect && reflect < midpoint+5){
        color = GRAY;
    }else if(reflect >= 95){
        color = WHITE;
    }else if(reflect <= 35){
        color = BLACK;
    }else if(midpoint-10 < reflect){
        color = NEARWHITE;//白
    }else if(reflect < midpoint+10){
        color = NEARBLACK;
    }else{
        color = RED;
    }
